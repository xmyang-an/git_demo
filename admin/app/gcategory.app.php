...
class GcategoryApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            $data = array(
				...
				'eval_tips' => str_replace('，',',',$_POST['eval_tips']),
                'eval_templates'    => str_replace('，',',',$_POST['eval_templates']),
            );
			if (!empty($_FILES['category_image']))
            {
				$category_image       =   $this->_upload_image();
            	$category_image && $data['category_image'] = $category_image;
			}
            /* 检查名称是否已存在 */
    ...
    function ajax_col()
    {
       ...
       $data   = array();
       if (in_array($column ,array('cate_name', 'if_show', 'sort_order','groupid')))
       {
           $data[$column] = $value;
           if($column == 'cate_name')
           {
		   ...
		   if (in_array($column ,array('if_show'))){
			   $id = $this->_gcategory_mod->get_descendant_ids($id);
		   }
    ...
    function batch_edit()
    {
        ...
        else
        {
			...
			$_POST['if_show'] >= 0 && $data['if_show'] = $_POST['if_show'] ? 1 : 0;
			$data['parent_id'] = intval($_POST['parent_id']);
			!empty($data) && $this->_gcategory_mod->edit($ids, $data);
    ...
    function _upload_image()
    {
        ...
        return $uploader->save('data/files/mall/category_image', $uploader->random_filename());
    }
}
