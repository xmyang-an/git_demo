...
//            $data['site_postcode']          = $_POST['site_postcode'];
			...
			$data['weixinkey']  =  empty($_POST['weixinkey']) ? array() : $_POST['weixinkey'];
			$data['weixinminkey']  =  empty($_POST['weixinminkey']) ? array() : $_POST['weixinminkey'];
            $model_setting->setAll($data);
...
//            $data['auto_evaluate']      = $_POST['auto_evaluate'];
    ...
    {
        $model_setting = &af('settings');
        $setting = $model_setting->getOne('alioss'); 
        if (!IS_POST)
        {
            $this->assign('setting', $setting);
            $this->display('setting.oss.html');
        }
        else
        {
            $data['alioss'] = array(
            ...
            $model_setting->setAll($data);
            $this->json_result(array('rel'=>1),'oss_setting_successed');
        }
    }
