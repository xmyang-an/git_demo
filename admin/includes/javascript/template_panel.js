...
$(function(){
    ...
    d.setWidth(270);
    d.show('center');
    $.get(REAL_BACKEND_URL + '/index.php?app=template&act=get_editor_panel&client=' + __CLIENT__ + '&page=' + __PAGE__, function(data){
...
function init_widget(widget)
{
	...
	//console.log($(widget).attr('name') + ' ' + !(typeof(__widgets[$(widget).attr('name')]) == "undefined"));
...
function save_page()
{
    ...
    /* 信息POST到处理脚本并显示结果*/
    $.post(REAL_BACKEND_URL + '/index.php?app=template&act=save&client=' + __CLIENT__ + '&page=' + __PAGE__, $('#_edit_page_form_').serialize(), function(rzt){
...
function config_widget(widget)
{
    ...
    d.setWidth(400);
    d.show('center');
    $.get(REAL_BACKEND_URL + '/index.php?app=template&act=get_widget_config_form&id=' + _id + '&name=' + _name + '&client=' + __CLIENT__ + '&page=' + __PAGE__, function(rzt){
        var _form = '<div class="widget_config_form"><form enctype="multipart/form-data" method="POST" action="' + REAL_BACKEND_URL + '/index.php?app=template&act=config_widget&id='+_id+'&name='+_name+'&client=' + __CLIENT__ + '&page='+__PAGE__+'" target="_config_post_iframe_" id="_config_widget_form_"><div class="widget_config_form_body">' + rzt + '</div><div class="dialog_buttons_bar" style="margin-top:20px;"><input type="submit" class="btn1" value="' + lang.submit + '" /><input type="reset" class="btn2" value="' + lang.reset + '" /></div></form></div>';
...
function add_widget(){
    ...
    d.setContents('loading', {text: 'loading...'});
    d.show('center');
    $.getJSON(REAL_BACKEND_URL + '/index.php?app=template&act=add_widget&name='+$(this).attr('widget_name')+'&client=' + __CLIENT__ + '&page=' + __PAGE__, function(rzt){
        if (rzt.done)
        {
