...
class ArticleApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		{
			$acategories[$key]['children'] = $this->_acategory_mod->find(array('conditions' => 'parent_id='.$val['cate_id']));
		}
		...
		$this->assign('categoryName', $category['cate_name']);
    ...
    function _get_article_curlocal($cate_id)
    {
        ...
        if ($cate_id)
        {
			...
			{
				$curlocal[] = array(
					'text' => $category['cate_name'], 'ACC' => $category['code'], 'url' => 'index.php?app=article&amp;cate_id=' . $category['cate_id']);
			}
		}
        return $curlocal;
    }
