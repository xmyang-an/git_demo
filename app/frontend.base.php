...
class FrontendApp extends ECBaseApp
{
    ...
    function display_widgets($options)
    {
        ...
        /* 获取该页面的挂件配置信息 */
        $widgets = get_widget_config($this->_get_template_name(), $page, 'pc');
...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user','check_email_info','check_phone_mob')) && !in_array(APP, array('bind')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode(get_domain() . $_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
    ...
    function _curitem($item)
    {
		...
		else 
		{
			...
			//if($_SESSION['member_role'] == 'maintain') {
				//unset($member_menu['im_buyer'], $member_menu['im_seller']);
				//$this->assign('member_role', 'maintain');
			//}
			if($_SESSION['member_role'] == 'buyer_admin' || in_array(ACT,array_keys($member_menu['im_buyer']['submenu'])) || in_array(APP,array_keys($member_menu['im_buyer']['submenu'])) || (APP == 'refund' && ACT == 'index')) {
				unset($member_menu['im_seller'], $member_menu['promotool']);
				$this->assign('member_role', 'buyer_admin');
			}
			elseif ($_SESSION['member_role'] == 'promotool_admin'){
				unset($member_menu['im_buyer'],$member_menu['im_seller']);
				$this->assign('member_role', 'promotool_admin');
			}
			else
			{
				unset($member_menu['im_buyer'], $member_menu['promotool']);
    ...
    function _get_member_menu()
    {
        ...
        $menu['im_buyer'] = array(
            ...
            'submenu'   => array(
				...
				'setting'  => array(
                    'text'  => Lang::get('my_setting'),
                    'url'   => 'index.php?app=member&act=setting',
                    'name'  => 'my_setting',
                ),
				'bind'  => array(
                    'text'  => Lang::get('member_bind'),
                    'url'   => 'index.php?app=bind&act=search',
                    'name'  => 'bind',
                ),
                'deposit' =>array(
		...
		{
			$menu['im_buyer']['submenu']['my_integral'] = array(
        ...
        if ($this->visitor->get('manage_store'))
        {
			...
			{
				$menu['im_seller']['submenu']['msg'] = array(
					'text'  => Lang::get('msg'),
			...
			$menu['promotool']['submenu']['appmarket'] = array(
                    'text'  => Lang::get('appmarket'),
                    'url'   => 'index.php?app=appmarket',
                    'name'  => 'appmarket',
            );
