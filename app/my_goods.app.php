...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        {
            $goods_list[$key]['cate_name'] = $this->_goods_mod->format_goods_cate_name($goods['cate_id']);
        }
        ...
        /* 当前页面信息 */
        $this->_curlocal(LANG::get('my_goods'), 'index.php?app=my_goods',
                         LANG::get('goods_list'));
   ...
   function ajax_col()
   {
	...
	function ajax_spec_image()
    {
        ...
        if ($file['error'] == UPLOAD_ERR_OK && $file !='')
        {
            ...
            $uploader->root_dir(ROOT_PATH);
            $data = $uploader->save('data/files/store_' . $this->_store_id . '/spec_images', $uploader->random_filename());
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
								...
								'spec_image'       => html_script(trim($_POST['spec_image'][$key])),
								'sort_order'	=>intval($_POST['sort_order'][$key]),
                            );
                        }
                        else  // 新增的规格
                        {
    ...
    function _check_post_data($data, $id = 0)
    {
		if($id > 0)
		{
			...
			if(!empty($group))
			{
				$this->show_warning('无法完成修改：商品已被设置成正在进行的团购活动。');
				return;
			}
		}
