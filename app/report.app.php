    ...
    function index()
    {
		...
		if(!IS_POST)
		{
			...
			$this->import_resource(array(
            	'script' => 'webuploader/webuploader.js,webuploader/webuploader.compressupload.js'
			));
		...
		else
		{
			...
			$files = $_POST['file_id'];
			if(empty($content)){
				$this->show_warning('content_no_empty');
				return;
			}
			if(empty($files)){
				$this->show_warning('file_empty');
				return;
			}
			if(count($files) > 5){
				$this->show_warning('file_gt_5');
				return;
			}
			$data = array(
			...
			$this->show_message('add_report_ok','back_list','index.php?app=my_report');
			return;
		}
    }
