...
/**
 *    Streamlining comes from Sparrow PHP @ MiMall
 *
 *    @author    MiMall
 */
...
class Conf
{
    ...
    {
        $old_conf = isset($GLOBALS['MiMall_CONFIG']) ? $GLOBALS['MiMall_CONFIG'] : array();
        if (is_string($conf))
        {
        ...
        {
            $GLOBALS['MiMall_CONFIG'] = array_merge($old_conf, $conf);
        }
        else
        {
            $GLOBALS['MiMall_CONFIG'] = $conf;
        }
    }
    /**
     *    获取配置项
    ...
    static function get($key = '')
    {
        $vkey = $key ? strtokey("{$key}", '$GLOBALS[\'MiMall_CONFIG\']') : '$GLOBALS[\'MiMall_CONFIG\']';
...
{  
	if(preg_match("/^1[0-9][0-9]{1}[0-9]{8}$/",$mobile))
	{  
		return true;  
	}  
	else
...
/* CURL发送请求 */
function ecm_curl($url, $method = 'GET', $post = array(), $cacert_url = '', $input_charset = '', $postJSON = FALSE)
{
...
function cut_str($string, $startLen = 1, $endLen = 1)
{
	if (function_exists('mb_substr')) {
		if(!empty($string)) {
			...
			getXingString($string) . mb_substr($string, -$endLen, $endLen, CHARSET);
		}
	}
	return $string;
}
...
function logResult1($key = '', $word = '') {
	...
	flock($fp, LOCK_EX) ;
	fwrite($fp,$key."        At:".date("Y-m-d H:i:s",time())."[IP:".real_ip()."]\n".$word."\n");
	flock($fp, LOCK_UN);
	fclose($fp);
}
