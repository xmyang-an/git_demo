/**
 ...
 @Name：layim mobile 1.0 Beta 临时版
 @Author：贤心
...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  //聊天主模板
  var elemChatTpl = ['<div class="page-webim layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'{{# if(d.base.chatTitleColor){ }}'
    ,'<div class="layim-chat-title" style="background-color: {{d.base.chatTitleColor}};">'
      ,'<p><span class="layim-back psmb-icon-font" onclick="window.history.back();">&#xe628;</span>{{ d.data.name }}<span class="layim-chat-status"></span><span class="layim-message psmb-icon-font J_Message">&#xe61a;<ins></ins></span></p>'
    ,'</div>'
    ,'{{# } }}'
  ...
  var layimChat, layimMin, To = {}, popchat = function(data){
    ...
    return popchat.index = layer.open({
      ...
      ,success: function(elem){
        ...
        hotkeySend();
		//viewChatlog();
        viewChatlog(data); // 需要data.id从服务器读取会话记录，by mimall
  ...
  */
  ...
  //渲染本地最新聊天记录到相应面板（从服务器加载数据，这样才能确保各个终端读取的数据是一致的 by mimall）
  var viewChatlog = function(data){
    var local = layui.data('layim-mobile')[cache.mine.id] || {};
    var thatChat = thisChat(), chatlog = local.chatlog || {};
    var ul = thatChat.elem.find('.layim-chat-main ul');
	
	$.ajax({ url: REAL_SITE_URL +"/index.php?app=webim&act=getChatLog", dataType: "json", data: {id:data.id,limit:MAX_ITEM}, success: function(data){
		if(data.done) {
			//console.log(data.retval);
			layui.each(data.retval, function(index, item){
      			if(new Date().getTime() > item.timestamp && item.timestamp - (sendMessage.time||0) > 60*1000){
        			ul.append('<li class="layim-chat-system"><span>'+ item.timestamp +'</span></li>');
        			sendMessage.time = item.timestamp;
      			}
      			ul.append(laytpl(elemChatMain).render(item));
    		});
    ...
    ,extend: function(othis){
      ...
      layui.each(call['tool('+ filter +')'], function(index, item){
        item && item(function(content){
          focusInsert(thatChat.input[0], content)
        });;
      });
    }
