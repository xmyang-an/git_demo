...
ScreenLocker = {
    ...
    'lock'      : function(zIndex){
        ...
        this.masker = $('<div class="masker" style="display:none"></div>');
		...
		// 不允许背景滑动（非全屏情况下，遮罩层不满屏）
		if($(document).width() <= 640) {
			$(document.body).css({'position':'fixed', 'left' : 0, 'right' : 0, 'top' : 0});
		}
    },
    'unlock'    : function(){
        if (this.masker === null)
		...
		this.masker.fadeOut('slow', function(){
        	$(document.body).find('.masker').remove();
		});
		this.masker = null;
...
Dialog.prototype = {
    ...
    'show'          : function(pos){
        ...
        /* 显示对话框 */
        $(this.dom.wrapper).css('bottom', '-400%').show();
		$(this.dom.wrapper).animate({bottom: 0}, 'slow');
    },
    ...
    'close'         : function(){
		...
		//var oClick = $('*[gs_id='+this.id+']');
		//oClick.prop('disabled', true);// 先锁定，避免操作频繁弹出多个层，导致无法关闭
		$(this.dom.wrapper).animate({bottom: '-150%'}, 1000, 'swing', function(){
			$(this).remove();
    ...
    'setPosition'   : function(pos){
        ...
        if (typeof(pos) == 'string')
        {
			...
			// 如果内容框大于整个屏幕，则加上滚动条
			if(dialog_height > $(window).height() || $(this.dom.wrapper).hasClass('scroll')) {
				$(this.dom.wrapper).find('.dialog_content_scroll').css({height:($(window).height()-150)+'px', 'overflow-y':'scroll'});
			}
        }
        else
        {
...
$(function(){
	$('body').on('click', '*[ectype="dialog"]', function(event){
        ...
        var url = $(this).attr('uri') + '&dialog_id='+id;
        ...
        ajax_form(id, title, url, width, style, opacity, position);
        return false;
    });
