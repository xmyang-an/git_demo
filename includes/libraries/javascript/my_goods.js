$(function(){
	...
	$('body').on('click', '.spec_image b',function(){
		...
		i.addClass('no-pic');
		i.html('');
//		var file_name = $(this).siblings('img').attr('src');
//		var url = SITE_URL + '/index.php?app=my_goods&act=drop_spec_image';
//		$.getJSON(url,{'file_name':file_name},function(data){
//			if(data.done){
//				i.addClass('no-pic');
//				i.html('');
//			}else{
...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
		...
		if(item.spec_image){
			tpl.find('*[item="spec_image"]').append('<input type="hidden" name="spec_image['+ item.spec_id +']" value="' + item.spec_image + '" /><img height="20" width="20" src="' + item.spec_image + '"/>');
		}else{
