...
function update_gselector_list(data)
{
	...
	if(data.goods_list.length != 0) {
		$.each(data.goods_list,function(i,item){
			html += '<ul class="clearfix"><li class="col-1 center clearfix"><div class="pic float-left"><a href="index.php?app=goods&id='+item.goods_id+'" target="_blank"><img width="40" height="40" src="'+item.default_image+'" /></div><div class="desc float-left"><a href="index.php?app=goods&id='+item.goods_id+'" target="_blank">'+item.goods_name+'</a></div></li><li class="col-2"><span class="price">'+item.price+'</span></li><li class="col-3">'+item.stock+'</li><li class="col-4 center"><a href="javascript:;" class="J_GselectorAdd btn-gselector-add" goods_name="'+item.goods_name+'" goods_id="'+item.goods_id+'">'+lang.selected+'</a></li></ul>';
		});
	}
	else {
...
function showPage(page)
{
	...
	$.getJSON('index.php?app=seller_growbuy&act=gselector', {'goods_name':goods_name,'sgcate_id':sgcate_id, 'page':page},function(data){
		if(data.done){
...
{
	$.getJSON('index.php?app=seller_growbuy&act=query_goods_info',{'goods_id':goods_ids},function(data){
		if(data.done){
			var goods_list = data.retval.goods_list;
			$('*[ectype="goods_list"]').html('');
			$.each(goods_list,function(i,item){
				$('*[ectype="goods_list"]').append('<li class="clearfix"><p class="cell-input"><input name="growbuy[selected_ids][]" type="hidden" value="'+item.goods_id+'" /></p><p class="cell-thumb float-left"><a href="index.php?app=goods&id='+item.goods_id+'" target="_blank"><img src="'+item.default_image+'" width="50" height="50" /></a></p><p class="cell-title float-left"><a href="index.php?app=goods&id='+item.goods_id+'" target="_blank">'+item.goods_name+'</a></p><p class="J_getPrice cell-price float-left" price="'+item.price+'">'+item.price+'</p><p class="cell-action float-left"><a class="J_MealDel" href="javascript:;">'+lang.drop+'</a></p></li>');
			});
		}
	});
}
...
function getLangMessage(code)
{
	...
	Lang['add_records']   = '您还没有添加商品';
	Lang['records_error'] = '您最多只能添加一件商品';
