...
class Tree extends Object
{
	...
	function getArrayListAll($root = 0 , $layer = NULL,$otherField = array())
    {
        $data = array();
        foreach ($this->child[$root] as $id)
        {
            ...
            $data[$id] = array('id' => $id, 'value' => $this->getValue($id), 'children' => $this->child[$id] ? $this->getArrayListAll($id , $layer,$otherField) : array());
			if(!empty($otherField) && is_array($otherField)){
				foreach($otherField as $val){
					$data[$id][$val] = $this->data[$id][$val];
				}
			}
        }
