...
class GcategoryBModel extends GcategoryModel
{
	...
	function get_parents($cate_id)
	{
		$this->_get_parents($result,$cate_id);
		return $result;
	}
	
	function _get_parents(&$result,$cate_id)
	{
		$cate = $this->get(intval($cate_id));
		$result[] = $cate['cate_id'];
		if($cate['parent_id']>0)
		{
			$this->_get_parents($result,$cate['parent_id']);
		}
	}
