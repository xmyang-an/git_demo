...
class DefaultPassportUser extends BasePassportUser
{
    ...
    function register($user_name, $password, $email, $local_data = array())
    {
		...
		if(isset($_SESSION['referid']) && $_SESSION['referid'])
		{
            $local_data['referid'] = $_SESSION['referid'];
        }
    ...
    function auth($user_name, $password)
    {
        ...
        {
			// 如果用户名验证不通过，则尝试是否为手机+密码登录（此功能不能兼容UC）
			$member_mod =& m('member');
			$info = $member_mod->get("phone_mob='{$user_name}' OR email='{$user_name}'");
    ...
    function check_username($user_name)
    {
		...
		// 为了兼容手机+密码登录，已存在的手机号也不能给验证通过（此功能不能兼容UC）
		$info = $model_member->get("phone_mob='{$user_name}'");
        if (!empty($info))
        {
            $this->_error('user_exists');
