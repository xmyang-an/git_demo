...
class Wxpay_h5Payment extends BasePayment
{
	/* 微信统一支付网关地址 */
    ...
    {
		// 如果不是手机的浏览器(非微信端)访问，则跳转到提示页面
		if((Psmb_init()->check_view_device('', false) == false) || isWeixin())
		{
			$this->_error('visitorinmobilebrowser');
			return false;
		}
        ...
        elseif(!$unifiedOrderResult["mweb_url"])
        {
		...
		//从统一支付接口获取到mweb_url
		$url = $unifiedOrderResult["mweb_url"].'&redirect_url='.urlencode($this->_create_return_url($payTradeNo));
		header("Location:$url");
    }
    ...
    {
        return SITE_URL .'/mobile/index.php?app=paynotify&act=wxWaitPay&payTradeNo='.$payTradeNo;
    }
    ...
    {
        return SITE_URL .'/includes/payments/wxpay_h5/notify_url.php';
    }
