...
class DefaultApp extends InstallerApp
{
    ...
    function config_done()
    {
        ...
        else
        {
            ...
            /* 不同意强制安装，则显示错误 */
            if ($has_samedb && empty($_POST['force_install']))
            {
                $this->_doing = $this->_done;
                $this->assign('has_samedb', true);
                $this->config();
    ...
    function install_done()
    {
        ...
        foreach ($sqls as $sql)
        {
            $sql = replace_prefix('swd_', $db_prefix, $sql);
        ...
        $sqls[] = "INSERT INTO `swd_member`(user_name, email, password, reg_time) VALUES('{$admin_name}', '{$admin_email}', '{$password}', " . gmtime() . ")";
        foreach ($sqls as $sql)
        {
            $rzt = mysql_query(replace_prefix('swd_', $db_prefix, $sql), $con);
            if (!$rzt)
            {
