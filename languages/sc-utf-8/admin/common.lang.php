<?php

return array(
	'colon' => '：',
	'admin_backend' => '商城后台管理中心',
	'msg' => '手机短信',
	'msg_manage' => '手机短信管理',
	'waybill' => '运单模板',
	'no_such_item' => '没有该项内容',
	'order_stat' => '订单统计',
	'batch_del'  => '批量删除',
	'batch_edit' => '批量编辑',
	'login' => '登&nbsp;&nbsp;录',
	'back_home' => '返回首页',
	'forget_pssword' => '忘记密码',
	'all_orders'			=>  '所有订单',
	'appmarket' 			=>  '应用市场',
	'deposit'				=>	'预存款',
	'refund_manage' 		=>  '退款管理',
	'deposit_manage'		=>	'预存款管理',
	'system_setting'		=>	'系统设置',
	'credit'				=>	'信用卡',
	'debit'					=>	'储蓄卡',
	'wait_admin_verify'		=>	'等待管理员审核',
	'success'				=>	'交易成功',
	'pending'				=>	'等待付款',
	'withdraw'				=>	'提现',
	'recharge'				=>	'充值',
	'edit_ok'				=> 	'编辑成功',
	'edit_error'			=>	'编辑失败',
	'add_ok'				=>	'添加成功',
	'add_error'				=>	'添加失败',
	'credit'				=>	'信用卡',
	'debit'					=>	'储蓄卡',
	'drop_ok'				=>	'删除成功',
	'drop_failed'			=>	'删除失败',
	'remark'				=>	'备注',
	'chargeback'			=>	'收费',
	'monthbill' 			=>  '月账单',
	'download_monthbill'    =>  '下载月账单',
	'select_month' 			=>	'请输入账单月份',
	
	'system'				=>  '系统',
	'refund' 				=>  '退款管理',
	'refundin'				=>	'退款维权',
	'refunddiff'			=>  '退款差价',
	'system_customer' 		=>  '平台客服',
	'trade_fee'				=>	'交易服务费',
	'transfer_fee'			=>	'转账服务费',
	'brokerage_fee'			=>	'佣金费',
	'rebate_fee'			=>	'赚取佣金',
	'chargeback'			=>	'收费',
	'charge' 				=>  '收费',
	'defray'				=>	'支付',
	'system_recharge'		=>	'系统充值',
	'system_chargeback'		=>	'系统扣费',
	'platform'				=>	'平台',
	'alipay' 				=>	'支付宝',
	'online_recharge' 		=>  '在线充值',
	'income'				=>	'收入',
	'outlay'				=>	'支出',
	'transfer'				=>	'转账',
	'refundin'  			=>  '退款维权',
	'refunddiff'			=>  '退款差价',
	'buygoods'				=>	'购物',
	'sellgoods'				=>	'卖出商品',
	'and_more'				=>	'等多件',
	'admin_agree_refund_content_change' => '%s已处理本次退款，处理结果为：商品退款金额为：%s 元，退运费金额: %s 元。<br> %s',
	'admin_agree_refund_order_status_change' => '平台客服处理退款，该订单中的所有商品都处于处理完毕状态，故订单状态自动转为已完成',
	
	'channel_manage' => '频道页管理',
	'ultimate_store'=>'推荐店铺管理',
	'props_manage'=>'属性管理',  
    'distribution_props' => '分配属性',
    'goods_props' => '商品属性',
	'integral_manage'=>'会员积分', 
	'integral_mall'=>'积分商城', 
	'integral_setting' =>'积分设置',
	'integral_list'=>'积分商品列表',
	'edit_integral_setting_successed'=>'编辑积分设置成功。',
	'exchange_rate'=>'积分兑换比率', 
	'exchange_rate_note'=>'例如：金钱：积分=1:1，则填写 1，金钱：积分=1:100，则填写 0.01。默认为0.1',
	'register_integral'  => '注册积分',
	'integral_setting' =>'积分设置',
	'register_has_integral'=>'注册送积分',
	'open_integral'=>'开店积分',
	'sign_in_integral' => '签到积分',
	'buying_has_integral'=>'购物积分',
	'selling_has_integral'=>'销售积分',
	'buying_pay_integral'=>'抵扣货款',
	'return_integral_for_cancel_order' => '订单取消，退回冻结的积分',
    'admin_handle'=>'管理员操作',
	'return_integral'=>'退回积分',
	'open_integral'=>'开店赠送积分',
	'buying_integral'=>'购买商品送积分',
	'store_sgrade'=>'店铺等级',
	'give_integral' =>'赠送积分比率（所赠积分即商品价格乘以该比率）',
	'register_integral_note'=>'用户注册后免费赠送积分值，默认为0',
	'open_integral_note'=>'用户开店后免费赠送积分值，默认为0',
	'sign_in_integral_note' => '用户每天登陆获得的积分值，默认为0',
	'integral_enabled'  => '是否启用积分功能',
	'open_has_integral'=>'开店获得赠送积分',
	
	'integral_finished' => '完成',
	'integral_frozen' => '冻结',
	'integral_cancel' => '取消',
	
    'username'  =>  '用户名',
    'password'  =>  '密&nbsp;&nbsp;&nbsp;码',
    'captcha'   =>  '验证码',
    'sort_order'=>  '排序',
    'handler'   =>  '操作',
    'manage'    =>  '管理',
    'add'       =>  '新增',
    'edit'      =>  '编辑',
    'drop'      =>  '删除',
    'batch_edit'=>  '批量编辑',
    'export'    =>  '导出',
    'import'    =>  '导入',
    'no_data'   =>  '没有符合条件的记录',
    'submit'    =>  '提交',
    'reset'     =>  '重置',
    'admin_nav' =>  '后台导航',
    'go_back'   =>  '返回上一页',
    'auto_redirect' => '若不选择将自动跳转',
    'sysmsg' => '系统提示',
    'system_message' => '系统消息',
    'has_error' =>  '非常抱歉，在处理您的请求时出现了一些错误，如下：<br />',
    'required_field'    => '有必填项未填',
    'data_not_enough'   => '所需数据不足',
    'no_valid_data' => '无有效数据，添加失败',
    'length_error'    =>  '有字符长度不符项',
    'drop_confirm'  => '您确定要删除它吗？',
    'enable'        => '启用',
    'disable'       => '禁用',
    'config'        => '配置',
    'yes'        => '是',
    'no'        => '否',
    'open'        => '开启',
    'close'        => '关闭',
    'disable_confirm'   => '您确定要禁用它吗？',
	'enable_confirm'   => '您确定要启用它吗？',
    'install'       => '安装',
    'uninstall'     => '卸载',
    'uninstall_confirm' => '您确定要卸载它吗？',
    'no_such_module'    => '没有该模块',
    'module_disabled'   => '模块已被禁用',
    'author'            => '作者',
    'version'           => '版本',
    'author_website'    => '作者链接',
    'title'         =>  '标题',
    'name'          =>  '名称',
    'link'          =>  '链接',
    'logo'          =>  '图片标识',
    'recommend'     =>  '推荐',
    'back_list'         => '返回列表',
    'continue_add'      => '继续新增',
    'login_required'    => '您需要登录后才能使用本功能',
    'logout_successed'  => '退出登录成功。',
    'login_successed'   => '登录成功。',
    'go_to_admin'       => '转到后台首页',
    'select_pls' => '请选择...',
    'if_show' => '显示',
    'query' => '查询',
    'autov_length_gt_max'   => '字符串长度超出限制',
    'autov_length_lt_min'   => '字符串长度不足',
    'unrecommend'       =>  '取消推荐',
    'if_recommended'    =>  '是否推荐',
    'recommend_confirm' =>  '您确定要推荐吗',
    'unrecommend_confirm' =>  '您确定要取消推荐吗',
    'not_allowed_type' => '您上传的文件格式不正确',
    'not_allowed_size' => '您上传的文件大小超过了允许值',
    'dir_doesnt_exists' => '上传文件存放目录无法创建，请联系站长检查相应权限',
    'no_ftp_server' => '未指定ftp服务器，请联系站长解决此问题',
    'view'          => '查看',
    'captcha_faild'     => '验证码输入错误',
    'login_error' => array(
        '-1' => '您输入的用户名或密码不正确，请重新输入',
        '-2' => '您的账户无法使用，可能是因为用户名重复或不符合规则',
    ),
    'login_failed'      => '登录失败，用户名和密码不匹配',
    'login_successed'   => '登录成功。',
    'logout_successed'  => '退出登录成功。',
    'no_permission'     => '您没有被授予相应的权限，无权访问',
    'not_admin'         => '您不是管理员，不能登录',
    'has_login'         => '您已经登录了',
    'prev_page' => '上一页',
    'next_page' => '下一页',
    'first_page' => '首页',
    'last_page' => '末页',
    'turn_page' => '翻页',
    'total_item' => '共 %s 个项目',
    'order_pending'     => '待付款',
    'order_accepted'   => '待发货',
    'order_canceled'    => '已取消',
    'order_submitted'   => '已提交',
    'order_shipped'     => '已发货',
    'order_finished'    => '已完成',
	
	// 针对交易的状态
	'TRADE_PENDING'				=> '等待买家付款',
	'TRADE_ACCEPTED'    		=> '买家已付款',
	'TRADE_SUBMITTED'			=> '等待卖家发货',// 货到付款的订单
	'TRADE_SHIPPED'				=> '卖家已发货',
	'TRADE_SUCCESS'				=> '交易完成',
	'TRADE_CLOSED'				=> '交易关闭',
	'TRADE_WAIT_ADMIN_VERIFY' 	=> '等待系统审核',
	
	// 交易类型
	'PAY'				=> '在线支付',
	'TRANSFER'			=> '转账',
	'SERVICE'			=> '服务费',
	'WITHDRAW'			=> '提现',
	'RECHARGE'			=> '充值',
	
	/*针对退款的状态*/
	'REFUND_SUCCESS'				=> '退款成功',
	'REFUND_CLOSED'					=> '退款关闭',
	'REFUND_WAIT_SELLER_AGREE'		=> '买家申请退款，等待卖家同意',
	'REFUND_SELLER_REFUSE_BUYER'	=> '卖家拒绝退款，等待买家修改中',
	'REFUND_WAIT_ADMIN_AGREE'		=> '卖家同意退款，等待管理员审核',
	'REFUND_WAIT_SELLER_CONFIRM'	=> '退款申请等待卖家确认中',
	
	'draw_return'	=> '提现退回',
	
    'export_data' => '导出数据',
    'import_data' => '导入数据',
    'back' => '返回',
    'note_for_export' => '您的商城数据是以 utf-8 编码存储的，导出时是否需要把数据转换为 %s 编码？',
    'reason_for_convert' => '如果您导出的文件需要用 excel 打开，请选择“是”，否则打开时可能显示乱码；如果您导出的文件只是为导入使用，建议不要转换，这样可以节省转换编码的时间。',
    'if_convert' => '是否转换编码',
    'select_file' => '请选择文件',
    'select_charset' => '请选择文件编码',
    'note_for_import' => '如果文件较大，建议您先把文件转换为 %s 编码，这样可以避免转换编码时耗费时间',
    'note_for_file' => '如果导入速度较慢，建议您把文件拆分为几个小文件，然后分别导入',
    'import_ok' => '导入数据成功。',
    'file_format' => '文件格式',
    'return'      => '返回',
    'store_name'    => '店铺名称',
    'cancel_query' => '撤销检索',
    'checkall'  => '全选',
    'db' => '数据库',
    'default_site_description' => '这是一个用MiMall架设的网上商城',
    'default_closed_reason' => '商城维护中，暂时关闭，请稍候访问。',
    'page_close' => '关闭页面',
    'if_admin' => '是否是管理员',
    'bat_upload' => '批量上传',
    'uploaded_files' => '已传图片',
    'insert_editor' => '插入编辑器',
    'upload_types' => '图片上传',
    'com_upload' => '普通上传',
    'button_text' => '选择本地图片(每张限%s)',
    'upload' => '上传',
    'canceled' => '已取消',
    'data_call' => '数据调用',
    'auth_failed'           => '密码验证失败',
    'user_exists'           => '用户已存在',
    'cannot_add_myself'     => '无法添加自己',
    'invalid_user_name'     => '无效的用户名',
    'blocked_user_name'     => '用户名中包含不允许注册的词语',
    'email_error'     => '电子邮箱错误',
    'blocked_email'     => '您的电子邮箱不允许注册',
    'email_exists'     => '该电子邮箱已被注册',
    'no_such_user'     => '没有指定的用户',
    'user_protected'     => '该用户受保护',
    'unknow_error'     => '未知错误',
    'uc_drop_user_failed'     => '从UC中删除用户失败',
    'password_error'     => '密码错误',
    'answer_error'     => '安全提问答案错误',
    'close_store_notice' => '关闭店铺通知',
    'your_store_is_closed' => '您的店铺已被关闭，原因是：%s',
    'close_reason' => '店铺已到期',
    'user_notice' => '会员通知',
    'comma' => array('，', '，'),
    'cannot_sent_to_myself' => '不能给自己发信息',
    'cancel_reason' => '取消原因',
    'view_detail' => '，<a target="_blank" href="%s">查看详情</a>',
    'feed_order_created' => array(
        'title'     => '{actor} 在店铺 {store} 购买了商品',
        'body'      => '',
        'name'      => '订单生成',
    ),
    'feed_store_created'    => array(
        'title'     => '{actor} 在 {mall_name} 开通了店铺 {store}',
        'body'      => '',
        'name'      => '店铺开通',
    ),
    'feed_goods_created'    => array(
        'title'     => '{actor} 添加了商品 {goods}',
        'body'      => '',
        'name'      => '添加商品',
    ),
    'feed_goods_collected'    => array(
        'title'     => '{actor} 收藏了商品 {goods}',
        'body'      => '',
        'name'      => '收藏商品',
    ),
    'feed_store_collected'    => array(
        'title'     => '{actor} 收藏了店铺 {store}',
        'body'      => '',
        'name'      => '收藏店铺',
    ),
    'feed_goods_evaluated'    => array(
        'title'     => '{actor} 给了商品 {goods} {evaluation}',
        'body'      => '评价：{comment}',
        'name'      => '评价商品',
    ),
    'goods_share' => '商品分享',
    'editable' => '单击可以编辑',
    'baidu_shoucang' => '百度收藏',
    'qq_shuqian' => 'QQ书签',
    'renrenwang' => '人人网',
    'kaixinwang' => '开心网',
    'share_link' => '分享链接',
    'not_csv_file' => '您选择的文件不是CSV格式',
	
	'your_account_has_locked' => '您的账户已经被锁定，请联系平台管理员。',
	'batch_del_for_selected'	=> '将选定行数据批量删除',
	'distribution_rate' => '分销比率',
	'refund_apply_warn_seller' => '买家退款提醒卖家短信',
	'seller_agree_refund_warn_buyer' => '退款成功提醒买家短信',
	'sms_warn_seller' => '您店铺有一笔新的退款申请，订单号为[%s]，请联系买家及时处理。',
	'sms_warn_buyer_refund_success' => '您的订单号为[%s]退款已处理完毕，请注意核实。',
	'sms_warn_seller_refund_success' => '您店铺订单号为[%s]的退款已处理完毕，如有疑问请联系平台。',
	
	'webim' => '客服管理',
	'imforbid' => '禁用IM',
	
	'mobile_terminal' => '移动端',
	
	'weixinmall' 	=> '微商城',
	'wxsetting' 	=> '微信设置',
	'wxbind'		=> '微信绑定',
	'wxmenu' 		=> '自定义菜单',
	'wxreply' 		=> '自动回复',
	'operation'	=> '操作',
	'add_data'	=> '新增数据',
	'no_selected_item'	=> '没有选择操作项',
	'no_empty'	=> '此项不能为空',
	'report_manage'	=> '举报管理',
	'operation_tip'	=> '操作提示',
	'explanation_tip'	=> '收起提示',
	'cashcard_manage'	=> '充值卡管理',
	'rechargecard' => '充值卡',
	
	'export_data_to_CVS' => '将选定行数据导出CVS文件',
	'download_all_data' => '您确定要下载全部数据吗？',
	'notice' => '提示',
	'evaluation_manage'=>'评价管理',
	'coupon' => '优惠券管理',
	'user_statistic' => '会员统计',
	'store_statistic' => '店铺统计',
	'delivery_manage' => '运费模板',
	'merchant_manage' => '商户管理',
	'material_manage' => '素材管理',
    'wx_mini_manage'  => '小程序管理',
    'xj_manage'=>'询价管理',
);

?>
