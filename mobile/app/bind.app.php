...
class BindApp extends MemberbaseApp
{
	...
	function search()
	{
		...
		$this->_get_curlocal_title('bindlist');
				...
				if(!empty($bind) && $member = $this->_member_mod->get($bind['user_id'])){
					...
					do {	 
						...
						if($token['portrait']){
							...
							file_put_contents($file_name, file_get_contents($token['portrait']));
							
							$portrait = str_replace(ROOT_PATH.'/', '', $file_name);
						}
						
						if($token['portrait']) $data['portrait']   = $portrait;
