...
class MealApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		if($goods_id) 
		{
			// 找出某个商品所有的搭配套餐（只需要套餐标题）
			$all = $this->_mealgoods_mod->find(array(
				'conditions' => 'status = 1 AND goods_id='.$goods_id,
				'join'       => 'belongs_to_meal',
				'fields'	 => 'this.meal_id, title',
			));
			...
			if(!$meal) {
				$first = current($all);
				$meal = $this->_meal_mod->findAll(array(
					'conditions' => 'status = 1 AND meal_id='.$first['meal_id'],
					'include' => array('has_mealgoods'),
				));
			}
		}
        ...
        $this->_config_seo('title', Lang::get('meal_detail') . ' - ' . Conf::get('site_title'));
