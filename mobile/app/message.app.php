...
class MessageApp extends MemberbaseApp
{
    ...
    function newpm()
    {
		...
		if(!IS_AJAX)
		{
        	...
        	$this->_config_seo('title',Lang::get('newpm') . ' - ' . Lang::get('member_center'));
			$this->_get_curlocal_title('newpm');
        	$this->display('message.box.html');
		}
		else
		{
			$messages = $this->_list_message('newpm', $this->visitor->get('user_id'));
			
			// 必须加 array_values() js遍历顺序才对
			$data = array('result' => array_values($messages), 'totalPage' => $page['page_count']);
    ...
    function privatepm()
    {
		...
		if(!IS_AJAX)
		{
        	...
        	$this->_config_seo('title',Lang::get('privatepm') . ' - ' . Lang::get('member_center'));
    ...
    function systempm()
    {
		...
		if(!IS_AJAX)
		{
        	...
        	$this->_config_seo('title',Lang::get('systempm') . ' - ' . Lang::get('member_center'));
			$this->_get_curlocal_title('systempm');
        	$this->display('message.box.html');
		}
		else
		{
			$messages = $this->_list_message('systempm', $this->visitor->get('user_id'));
    ...
    function announcepm()
    {
        ...
        $this->_config_seo('title', Lang::get('announcepm') . ' - ' . Lang::get('member_center'));
        $this->display('message.box.html');
    }
    /**
     *    发送短消息
    ...
    function send()
    {
        if (!IS_POST){
            ...
            $this->import_resource(array(
                'script' => 'mobile/jquery.plugins/jquery.form.min.js',
            ));
			$this->_get_curlocal_title('send_message');
            $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('send_message'));
    ...
    function _list_message($pattern, $user_id)
    {
        ...
        {
			$messages[$key]['last_update'] = local_date('Y-m-d H:i:s', $message['last_update']);
