...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		else
		{
			...
			/* 分页信息 */
			$page = $this->_get_page(intval($_GET['pageper']) > 0 ? intval($_GET['pageper']) : NUM_PER_PAGE);
    ...
    function store()
    {
		...
		if(!IS_AJAX)
		{
			if($scategorys = $this->_list_scategory())
			{
				foreach($scategorys as $k=>$v) {
					$scategorys[$k]['count'] = count($v['children']);
				}
				$this->assign('scategories', $scategorys);
			}
			$this->assign('regions', $model_store->list_regions());
			
			$baidukey = Conf::get('baidukey.browser');
			$this->assign('baidukey', $baidukey);
			
			$this->headtag('<script type="text/javascript" src="{lib file=mobile/jquery.plugins/jquery.infinite.js}"></script><script type="text/javascript" src="{lib file=mobile/search_store.js}"></script><script type="text/javascript" src="https://api.map.baidu.com/api?v=2.0&ak='.$baidukey.'"></script><script type="text/javascript" src="https://api.map.baidu.com/library/GeoUtils/1.2/src/GeoUtils_min.js"></script>');
			...
			$this->_get_curlocal_title(Lang::get('searched_store'));
		...
		else
		{
			...
			$lat = $_COOKIE['pointLat'];
			$lng = $_COOKIE['pointLng'];
			if($lat && $lng)
			{
				...
				$bound = intval($_GET['bound']) ? intval($_GET['bound']) : '';
				if($bound)
				{
					$map_conditions = ' AND '.$mapPiontCaculation.' < '.$bound;
				}
			}
			...
			$stores = $model_store->find(array(
				'conditions'  => 'state = ' . STORE_OPEN . $map_conditions.$condition_id . $conditions,
				'limit'   =>$page['limit'],
				'order'   => empty($_GET['order']) || !in_array($_GET['order'], $orders) ? $default_order : $_GET['order'],
			...
			foreach ($stores as $key => $store)
			{
				...
				//$stores[$key]['collect'] = db()->getOne("SELECT count(*) FROM ".DB_PREFIX."collect c WHERE type='store' AND item_id=".$store['store_id']);
	...
	function form()
	{
		...
		// 搜索商品，店铺
		$act = trim($_GET['searchType']) ? trim(html_script($_GET['searchType'])) : 'index';
		if(!in_array($act, array('index', 'goods', 'store'))) {
			$act = 'index';
		}
		$searchUrl = site_url() . '/index.php?app=search&act='.$act;
		$params = array('app' => 'search', 'act' => $act);
		...
		{
			// 商品
			if(!in_array($act, array('store'))) {
				$keywords =  $this->_get_hot_keywords();
			}
			// 店铺
			else 
			{
		...
		// 搜索店铺商品
		elseif($store_id) {
			$searchUrl = site_url() . '/index.php?app=store&act=search&id='.$store_id;
			$params = array('app' => 'store', 'act' => 'search', 'id' => $store_id);
		...
		if($keywords)
		{
			...
			{
				$formKeyword[] = array('text' => $v, 'url' => $searchUrl . '&keyword='.$v);
			}
			$this->assign('formSearchBoxKeyword', $formKeyword);
		}
		$this->assign('formSearchBoxParams', $params);
		
		$this->_config_seo(array('title' => Lang::get('searched_'.$act) . ' - ' . Conf::get('site_title')));
		$this->_get_curlocal_title('searched_'.$act);
    ...
    function groupbuy()
    {
		...
		else
		{
			...
			$data = array('result' => array_values($groupbuy_list), 'totalPage' => $page['page_count']);
			echo json_encode($data);
		}
    }
    ...
    function _get_filter($param)
    {
        ...
        if ($filters === null)
        {
			...
			if(isset($param['cate_id'])) {
				...
				$filters['gcategory'] = array('key' => 'cate_id', 'name' => Lang::get('gcategory'), 'value' => $gcategory['cate_name']);
			}
        }
        return $filters;
    }
