...
class WebimApp extends MallbaseApp
{
	...
	function friend()
	{
		$friendList = $this->_getList();
		$this->assign('friendList', $friendList);
		$this->display('webim.friend.html');
	}
	
	// 这个是给ajax调用的，目前只有PC用到，先保留
	function getList()
	{
		$friendList = $this->_getList();
		
		echo ecm_json_encode($friendList);
	}
	...
	function _getList()
	{
	   	...
	   	if($this->visitor->has_login)
	   	{
			...
			if($imlog)
			{
				...
				foreach($friendList as $friend)
				{
					...
					//找出当前好友跟我的最后发言
					$talk = $this->_webim_log->get(array(
						'conditions' => 'fromid='.$friend.' AND toid='.$this->visitor->get('user_id'),
						'order'   => 'logid DESC',
						'fields'  => 'formatContent',
					));
					$lastTalk = $talk['formatContent'];
					
					// 找出这个好友发给我的未读的信息量
					$unread = $this->_getTotalUnRead($friend, $this->visitor->get('user_id'));
					
					$list[] = array('username' => $username, 'id' => $friend, 'avatar' => $avatar, 'sign' => '', 'lastTalk' => $lastTalk, 'unread' => $unread);
				}
		...
		return array('code' => 0, 'msg'  => '', 'data' => $result);
	}
	...
	function getLog()
	{
		...
		$this->_get_curlocal_title('getlog');
	...
	function unread()
	{
		...
		$this->_webim_log->edit("logid={$logid} AND toid={$this->visitor->get('user_id')}", array('unread' => 1));
	...
	{
		$uid = intval($_GET['toid']);
		$result = $this->_getTotalUnRead(0, $uid);
		
		$this->json_result($result);
	}
	
	// 找出所有（或指定某个用户）发给我的未读信息总数
	function _getTotalUnRead($fromid = 0, $toid = 0)
	{
		$result = 0;
		
		$conditions = '';
		if($fromid) {
			$conditions .= ' AND fromid='.$fromid;
		}
		if($toid) {
			$conditions .= ' AND toid='.$toid;
		}
		
		$imlog = $this->_webim_log->find(array(
			'conditions' => 'unread=1'.$conditions,
			'fields'     => 'logid'
		));
		$imlog && $result = count($imlog);
