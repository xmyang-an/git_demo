	...
	function callback()
	{
		...
		if($result)
		{
			...
			$bind = $this->_bind_mod->get(array(
				'conditions'=>"unionid='".$unionid."' AND app ".db_create_in(array('weixin', 'miniprogram')), 'fields'=>'user_id,enabled'));
			...
			if($bind && $bind['user_id'] && $bind['enabled'] && (!$this->visitor->has_login || ($this->visitor->get('user_id') == $bind['user_id'])))
			{
				...
				$checkBind = $this->_bind_mod->get(array(
				'conditions'=>"unionid='".$unionid."' AND app='".$this->_app."'", 'fields'=>'user_id'));
				if(empty($checkBind))
				{
					//授权令牌
