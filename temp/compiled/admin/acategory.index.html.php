...
?>
            <tr>
                ...
                <td class="handler bDiv" style=" background:none; width:230px; text-align:left;">
                    ...
                    <a href="index.php?app=acategory&amp;act=edit&amp;id=<?php echo $this->_var['acategory']['cate_id']; ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>编辑</a> <a href="javascript:goConfirm('删除该分类将会同时删除该分类的所有下级分类，您确定要删除吗', 'index.php?app=acategory&amp;act=drop&amp;id=<?php echo $this->_var['acategory']['cate_id']; ?>',true);" class="btn red"><i class="fa fa-trash-o"></i>删除</a>
                    <?php endif; ?>
                    <?php if ($this->_var['acategory']['layer'] < $this->_var['max_layer'] && $this->_var['acategory']['parent_children_valid']): ?>
                    <?php if (! $this->_var['acategory']['code']): ?>
                    <?php endif; ?>
                    <a href="index.php?app=acategory&amp;act=add&amp;pid=<?php echo $this->_var['acategory']['cate_id']; ?>" class="btn green"><i class="fa fa-plus"></i>新增下级</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; else: ?>
        ...
        <tfoot>
            <tr class="tr_pt10">
                ...
                <td colspan="3" id="batchAction"><span class="all_checkbox mt5">
                    <label for="checkall_2">全选</label>
                    </span>&nbsp;&nbsp;
                    <div class="fbutton JBatchDel" style="border:0;" uri="index.php?app=acategory&act=drop" name="id"><div class="del" title="将选定行数据批量删除"><span><i class="fa fa-trash"></i>批量删除</span></div></div>
                </td>
                <?php endif; ?>
            </tr>
        </tfoot>
