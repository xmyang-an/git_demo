         ...
         $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
             if(data)
             {
                 ...
                 for(var i = 0; i < res.length; i++)
                 {
                     ...
                     {
                         add_child = "<a href='index.php?app=region&amp;act=add&amp;pid="+res[i].region_id+"' class='btn green'><i class='fa fa-plus'></i>新增下级</a>";
                     }
...
?>
        <tr class="">
            ...
            <td class="handler bDiv" style=" background:none; width:230px; text-align:left;">
                <a href="index.php?app=region&amp;act=edit&amp;id=<?php echo $this->_var['region']['region_id']; ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>编辑</a>
                <a href="javascript:goConfirm('删除该地区将会同时删除该地区的所有下级地区，您确定要删除吗', 'index.php?app=region&amp;act=drop&amp;id=<?php echo $this->_var['region']['region_id']; ?>',true);" class="btn red"><i class="fa fa-trash-o"></i>删除</a>
                <?php if ($this->_var['region']['layer'] < $this->_var['max_layer']): ?>
                <a href="index.php?app=region&amp;act=add&amp;pid=<?php echo $this->_var['region']['region_id']; ?>" class="btn green"><i class="fa fa-plus"></i>新增下级</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; else: ?>
        ...
        <tfoot>
            ...
            <?php if ($this->_var['regions']): ?>
                ...
                <td colspan="3" id="batchAction">
                    ...
                    <div class="fbutton JBatchDel" style="border:0;" uri="index.php?app=region&act=drop" name="id"><div class="del" title="将选定行数据批量删除"><span><i class="fa fa-trash"></i>批量删除</span></div></div>
                </td>
            <?php endif; ?>
            </tr>
        </tfoot>
